function handle_stream_state!(connection::HTTPConnection, frame, issend::Bool)
    ...
    if stream.state == IDLE
        ...
        ## type PROTOCOL_ERROR.
        if isrecv && (isa(frame, PriorityFrame) ||
                      isa(frame, HeadersFrame))
            return
        end
    ...
    if stream.state == RESERVED_LOCAL
        ...
        if isa(frame, HeadersFrame) && issend
        ...
        if issend && (isa(frame, PriorityFrame) ||
                      isa(frame, RstStreamFrame) ||
                      isa(frame, HeadersFrame))
            return
        end
    ...
    if stream.state == RESERVED_REMOTE
        ...
        ## "half-closed (local)".
        if isa(frame, HeadersFrame) && isrecv
            stream.state = HALF_CLOSED_LOCAL
            return
        end
        ...
        if issend && (isa(frame, RstStreamFrame) ||
                      isa(frame, WindowUpdateFrame) ||
                      isa(frame, PriorityFrame))
            return
        end
        ...
        if isrecv && (isa(frame, HeadersFrame) ||
                      isa(frame, RstStreamFrame) ||
                      isa(frame, PriorityFrame))
            return
        end
    ...
    if stream.state == HALF_CLOSED_REMOTE
        ...
        ## RST_STREAM frame.
        if (isa(frame, DataFrame) || isa(frame, HeadersFrame)) && frame.is_end_stream && issend
            stream.state = CLOSED
            return
        end
        ...
        if issend && isa(frame, PushPromiseFrame)
        ...
        if isrecv && (isa(frame, WindowUpdateFrame) ||
                      isa(frame, PriorityFrame) ||
                      isa(frame, RstStreamFrame))
            return
        end
    ...
    if stream.state == HALF_CLOSED_LOCAL
        ...
        ## RST_STREAM frame.
        if (isa(frame, DataFrame) || isa(frame, HeadersFrame)) && frame.is_end_stream && isrecv
            stream.state = CLOSED
            return
        end
        ...
        if isrecv && isa(frame, PushPromiseFrame)
            promised_stream = get_stream(connection, frame.promised_stream_identifier)
        ...
        ## RST_STREAM.
        if issend && (isa(frame, WindowUpdateFrame) ||
                      isa(frame, PriorityFrame) ||
                      isa(frame, RstStreamFrame))
            return
        end
    ...
    if stream.state == OPEN
        ...
        ## "half-closed (remote)".
        if (isa(frame, DataFrame) || isa(frame, HeadersFrame)) && frame.is_end_stream
            if issend
                stream.state = HALF_CLOSED_LOCAL
            else
                stream.state = HALF_CLOSED_REMOTE
